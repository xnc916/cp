...
import com.xiongyuan.lottery.homepage.other.CommUtil;
...
public class CustomGridView extends LinearLayout {
	...
	private void initView(Context context) {
		...
		LinearLayout textView = (LinearLayout) root.findViewById(R.id.container_ll);
	...
	public void notifyDataSetChange(boolean needAnim) {
		...
		for (int rowIndex = 0; rowIndex < rowNum; rowIndex++) {
			...
			for (int columnIndex = 0; columnIndex < CustomGroup.COLUMNUM; columnIndex++) {
				...
				ImageView ivIcon = (ImageView) root.findViewById(R.id.icon_iv);
				TextView tvName = (TextView) root.findViewById(R.id.name_tv);
				TextView tvContent = (TextView) root.findViewById(R.id.content_tv);
				LinearLayout rl = (LinearLayout) root.findViewById(R.id.container_ll);
				if (isValidateView) {
					final DargChildInfo tempChilid = mPlayList.get(itemInfoIndex);

					ivIcon.setImageResource(tempChilid.getResIconId());
					tvName.setText(tempChilid.getName());
					tvContent.setText(tempChilid.getContent());
				...
				if (columnIndex != CustomGroup.COLUMNUM - 1) {
					...
					view.setBackgroundResource(R.color.homeColorBackground);
